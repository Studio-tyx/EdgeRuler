...
RULES = [[7, 30.2, 1000, DELTA, 1],  [8, 1011.8, 1000, DELTA, 1], 
         [2, 30.0, 2000, DELTA, 2], [3, 23.8, 2000, DELTA, 2], [3, 29, 2000, DELTA, 2], 
         [3, 32, 3000, DELTA, 3], [2, 31, 3000, DELTA, 3], [5, 0.04, 3000, DELTA, 3], 
         [1, 50000, 4000, DELTA, 4], [8, 1012.2, 4000, DELTA, 4], [0, 4.0, 5000, DELTA, 5], 
         ]


RULE_FILE_NAME = ['IndoorHumidity_pct-30.2.txt', 'IndoorPressure_hPa-1011.8.txt', 
'OutdoorTemperature_C-30.txt', 'OutdoorHumidity_pct-23.8.txt','OutdoorHumidity_pct-29.txt', 
'OutdoorHumidity_pct-32.txt', 'OutdoorTemperature_C-31.txt', 'IndoorUV-0.04.txt', 
'OutdoorVisibleRay_Lux-50000.txt', 'IndoorPressure_hPa-1012.2.txt', 'OutdoorUV-4.txt', 
]
...
P = {1000: {4.0: 0.372, 6.0: 0.259, 8.0: 0.203, 12.0: 0.1872, 16.0: 0.1727},
    2000: {4.0: 0.739, 6.0: 0.525, 8.0: 0.416, 12.0: 0.3837, 16.0: 0.3539},
    3000: {4.0: 1.097, 6.0: 0.767, 8.0: 0.613, 12.0: 0.5654, 16.0: 0.5215},
    4000: {4.0: 1.442, 6.0: 1.018, 8.0: 0.803, 12.0: 0.7407, 16.0: 0.6832},
    5000: {4.0: 1.802, 6.0: 1.262, 8.0: 0.992, 12.0: 0.915, 16.0: 0.844},
    6000: {4.0: 2.183, 6.0: 1.523, 8.0: 1.189, 12.0: 1.0967, 16.0: 1.0116},
    7000: {4.0: 2.532, 6.0: 1.774, 8.0: 1.385, 12.0: 1.2775, 16.0: 1.1783},
    8000: {4.0: 2.875, 6.0: 2.021, 8.0: 1.577, 12.0: 1.4546, 16.0: 1.3417},
    9000: {4.0: 3.245, 6.0: 2.265, 8.0: 1.773, 12.0: 1.6354, 16.0: 1.5084},
    10000: {4.0: 3.598, 6.0: 2.535, 8.0: 1.974, 12.0: 1.8208, 16.0: 1.6794}}
...
def f(op, r):
    if r < 1 or r > 16:
        print("invalid r")
    return math.ceil(P[op][int(r)] * 10)    # horizon
    # return max(1, math.ceil(P[op][int(r)] * 30 - op / 250))
...
def sensor_data_filter(trigger_name, length):
    df = pd.read_csv('../../../data/INFOCOM/sensor_data/er_raw_data.csv', header=0, nrows=length)
...
def get_event_optimal():
    ...
    for i in range(len(RULES)):
        triggered_time = get_triggered_time(sensor_data_filter(SENSOR_NAME[RULES[i][0]], LENGTH), RULES[i][1])
        dataset.append([i, triggered_time])
    # for rule in RULES:
...
def get_event_dbp():
    sensor_events = []
    for j in range(len(RULE_FILE_NAME)):
        data = []
        events = []
        with open(f'../../../data/INFOCOM/dbp/{RULE_FILE_NAME[j]}', 'r') as file:
...
def collect_total_data(event):
    current = 0
    i = 0
    real_events = []
    real_event = []
    while current < event[-1][0]:
        if i < len(event) and current <= event[i][0] < current + HORIZON_LENGTH:
            real_event.append(event[i])
            i += 1
        else:
            current += HORIZON_LENGTH
            # if len(real_event) > MAX_EVENT_PER_HORIZON: real_event = real_event[:MAX_EVENT_PER_HORIZON]
            # if len(real_events) == 0:
            #     real_events.append(real_event)
            # else:
            #     if len(real_events[-1]) ==0 or abs(real_events[-1][-1][0] - current) > 10:
            #         real_events.append(real_event)
            real_events.append(real_event)
            # print(real_event)
            real_event = []
...
def get_truth():
    return collect_total_data(get_event_optimal())
    

def get_dbp_predict():
    return collect_total_data(get_event_dbp())

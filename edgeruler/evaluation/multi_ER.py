...
from greedy_min_latency import greedy_run
from greedy_min_latency import ignore_ddl_greedy
...
def compare_run():
    from set_up import LENGTH, HORIZON_LENGTH, R_CHOICE, get_truth, get_dbp_predict
    length = LENGTH
    horizon = HORIZON_LENGTH
    true_event = get_truth()
    pred_t = get_dbp_predict()
    pred_t = true_event
    ...
    for i in range(len(true_event)):
        ...
        if len(horizon_t) == 0:
            ...
            s_min, solution = greedy_run(horizon_ddl, horizon_t, horizon_cts, horizon_p, R_CHOICE, horizon_r.copy(), horizon_ids, horizon_delta)
            if len(solution) == 3: s_min, solution = delta_t_repredict(s_min, solution)
            from greedy_min_latency import MAX
            if s_min == MAX:
                no_solution = True
                s_min, solution = ignore_ddl_greedy(horizon_ddl, horizon_t, horizon_cts, horizon_p, R_CHOICE, horizon_r.copy(), horizon_ids, horizon_delta)
